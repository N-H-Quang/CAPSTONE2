...
function EditMovie() {
  ...
  const movie = movieData?.data.content;
  ...
  React.useEffect(() => {
    if (movie) {
      setFormData({
        ...
        moTa: movie.moTa,
        ngayKhoiChieu: new Date(movie.ngayKhoiChieu).toISOString().split("T")[0],
        hot: movie.hot,
  ...
  const updateMovieMutation = useMutation({
    mutationFn: (data: FormData) => adminApi.updateMovie(data),
    onSuccess: () => {
    ...
    onError: (error: unknown) => {
      ...
      toast.error(err?.response?.data?.content || "Failed to update movie");
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMovieMutation.mutate(formDataToSend);
  };
